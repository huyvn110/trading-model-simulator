...
function TestChartsComponent() {
    ...
    const dynamicStats = useMemo(() => {
        const modelStats = trades.reduce((acc, trade) => {
            const key = trade.modelKey;
            if (!acc[key]) {
                acc[key] = {
                    modelKey: trade.modelKey,
                    factorNames: trade.factorIds.map(getFactorName).sort(),
                    totalTrades: 0,
                    wins: 0,
                    losses: 0,
                    totalValue: 0,
            ...
            if (trade.result === 'win') {
                acc[key].wins++;
                acc[key].winValue += tradeValue;
                acc[key].totalValue += tradeValue;
            } else {
                acc[key].losses++;
                acc[key].lossValue += Math.abs(tradeValue);
                acc[key].totalValue -= Math.abs(tradeValue);
            }
            return acc;
                                ...
                                <Table size="small">
                                    <TableHead>
                                        <TableRow sx={{ background: 'linear-gradient(135deg, rgba(99, 102, 241, 0.15) 0%, rgba(139, 92, 246, 0.1) 100%)' }}>
                                            ...
                                            <TableCell align="center" sx={{ color: '#94a3b8', fontWeight: 600, borderBottom: '1px solid rgba(255,255,255,0.08)' }}>{viewMode}</TableCell>
                                            ...
                                            <TableRow key={stat.modelKey} sx={{ '&:hover': { bgcolor: isDark ? 'rgba(241,245,249,0.02)' : 'rgba(15,23,42,0.02)' } }}>
                                                ...
                                                <TableCell sx={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                                                    <Stack direction="row" spacing={0.5} flexWrap="wrap" useFlexGap>
                                                        {stat.factorNames.map((name: string) => (
